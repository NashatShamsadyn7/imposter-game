...
// ═══════════════ یاری IQ ـی ئۆنلاین ═══════════════
export async function iqCreateRoom(user, profile, settings) {
  need()
  let room
  for (let i = 0; i < 5; i++) {
    const code = genCode()
  ...
  if (!room) throw new Error('نەتوانرا ژوور دروست بکرێت')
  await iqJoinRow(room.id, user, profile, true)
  return room
}

async function iqJoinRow(roomId, user, profile, isHost) {
  await supabase.from('iq_players').upsert(
    {
      room_id: roomId,
      user_id: user.id,
      display_name: profile.display_name,
...
export async function iqJoinRoom(code, user, profile) {
  need()
  const { data: room } = await supabase
    ...
    .maybeSingle()
  if (!room) throw new Error('ژوور نەدۆزرایەوە')
  await iqJoinRow(room.id, user, profile, room.host_id === user.id)
  return room
}

export async function iqLeaveRoom(roomId, userId) {
  if (!supabase) return
  await supabase.from('iq_players').delete().eq('room_id', roomId).eq('user_id', userId)
}

export async function iqUpdateRoom(roomId, patch) {
  need()
  await supabase.from('iq_rooms').update(patch).eq('id', roomId)
}

export async function iqFetchRoom(roomId) {
  need()
  const { data } = await supabase.from('iq_rooms').select('*').eq('id', roomId).maybeSingle()
  return data
}

export async function iqFetchPlayers(roomId) {
  need()
  const { data } = await supabase
    .from('iq_players')
    .select('*')
...
export async function iqFetchAnswers(roomId) {
  need()
...
export async function iqSubmitAnswer(row) {
  need()
  // upsert تاکو دووبارە وەڵامدانەوە ڕێگری بکرێت
  await supabase.from('iq_answers').upsert(row, { onConflict: 'room_id,q_index,user_id', ignoreDuplicates: true })
}

export function subscribeIQRoom(roomId, handlers) {
  need()
  const channel = supabase
    .channel(`iq:${roomId}`)
    .on('postgres_changes',
      { event: '*', schema: 'public', table: 'iq_rooms', filter: `id=eq.${roomId}` },
      (p) => handlers.onRoom?.(p))
    .on('postgres_changes',
      { event: '*', schema: 'public', table: 'iq_players', filter: `room_id=eq.${roomId}` },
      () => handlers.onPlayers?.())
    .on('postgres_changes',
      { event: '*', schema: 'public', table: 'iq_answers', filter: `room_id=eq.${roomId}` },
      () => handlers.onAnswers?.())
    .subscribe()
  return () => supabase.removeChannel(channel)
}
