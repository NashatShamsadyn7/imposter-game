  ...
  return (
    <div className="mx-auto max-w-md px-4 py-6 pb-24">
      ...
      <Panel className="mb-5 text-center">
        <p className="mb-1 text-xs text-muted">وشەی نهێنی ({game.category.name})</p>
            ...
            <div key={r.user_id} className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 ${r.role === 'impostor' ? 'border-impostor/30 bg-impostor/8' : 'border-crew/25 bg-crew/5'}`}>
              <div className="flex-1">
                <p className="flex items-center gap-1.5 font-bold text-ink">
                  ...
                  {r.display_name}
                  {r.ejected && <span className="flex items-center gap-0.5 text-xs text-muted"><UserX className="h-3 w-3" /> دەرکرا</span>}
                </p>
                <p className="text-xs text-muted">
                  {r.role === 'impostor' ? 'ساختەکار' : 'دەستەی کەشتی'} · {r.votes} دەنگ · کۆ: {scores[r.user_id] || 0}
                </p>
              </div>
      ...
      <Button onClick={playAgain} className="w-full !py-4 !text-lg">
        <RotateCcw className="h-6 w-6" /> یاری دووبارە
      </Button>
    </div>
  )
}
